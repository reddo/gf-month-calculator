...
class GF_Field_Month_Calculator extends GF_Field {
	...
	public function get_field_input( $form, $value = '', $entry = null ) {
		...
		if ( $is_form_editor || $this->timeFormat != '24' ) {
			$am_text = esc_html__( 'AM', 'gf-month-calculator' );
			$pm_text = esc_html__( 'PM', 'gf-month-calculator' );
			$aria_label = esc_attr( 'AM/PM', 'gf-month-calculator' );
		...
		$hour_label = rgar( $hour_input, 'customLabel' ) != '' ? $hour_input['customLabel'] : esc_html__( 'HH', 'gf-month-calculator' );
		$minute_label = rgar( $minute_input, 'customLabel' ) != '' ? $minute_input['customLabel'] : esc_html( _x( 'MM', 'Abbreviation: Minutes', 'gf-month-calculator' ) );
...
// Register the Month Calculator field with the field framework.
GF_Fields::register( new GF_Field_Month_Calculator() );
